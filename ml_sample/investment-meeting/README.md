# Investment Meeting Simulator (MCP Enabled)

投資判断「ベテラン会議」シミュレーター。Google Gemini 2.5 Flash Lite と MCP (Model Context Protocol) を活用し、実際の市場データ（株価・ニュース）に基づいた多角的な議論を行う意思決定支援ツール。

## 🎯 プロジェクト概要

3人の異なる投資家エージェントが、**MCP ツールを使用して取得した最新の証拠**に基づき、**最低3往復の徹底的な議論**を行います。

### 主な機能
- **MCP 統合**: `yfinance` を通じたリアルタイム株価、指標（PER/PBR）、最新ニュースの取得。
- **マルチエージェント議論**: 性格の異なるエージェントによる反論・再反論ループ。
- **自動レポート生成**: 議論ログの集約、翻訳、および構造化 JSON 出力。

### エージェント

1. **グロース投資家 (Growth Investor)**
   - 市場の破壊的イノベーション、将来のキャッシュフロー、TAM を重視。
2. **バリュー投資家 (Value Investor)**
   - 安全域、PBR/PER、B/S の健全性、ダウンサイド保護を重視。
3. **データ重視アナリスト (Data Analyst)**
   - 最新の数値データ、業界ベンチマーク、客観的なリスク評価を重視。
...
├── src/
│   ├── __init__.py
│   ├── mcp_server.py       # MCP サーバー（株価・ニュース取得ツール）
│   ├── agents.py           # 投資家エージェント（MCP ツール対応）
│   ├── moderator.py        # 議論ロジック（3往復ループ制御）
│   ├── reporter.py         # レポート生成・翻訳
│   └── main.py             # エントリーポイント（MCP クライアント）
└── tests/
    ├── test_mcp.py         # MCP ツールの検証
    ├── test_agents.py      # エージェント動作
    └── test_moderator.py   # ディバートループの検証
```
...
1. **フェーズ1: データ収集と初期判断**
   エージェントが MCP ツールを使用して対象銘柄の最新指標（PER, PBR, 株価推移等）を取得し、意見を述べます。
2. **フェーズ2: 反論と再反論 (最低3往復)**
   他のエージェントの意見に対し、取得したデータを用いて論理的な弱点を指摘します。
3. **フェーズ3: レポート生成**
   議論を要約し、投資判断レポートを出力します。
