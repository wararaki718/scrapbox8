...
class FrobeniusCorrelationLoss(nn.Module):
    ...
    def forward(
        ...
        I = torch.eye(embeddings.size(0), device=embeddings.device)
        penalty = (correlation_matrix - I).pow(2).sum()
...
class DiversityInfoNCELoss(nn.Module):
    def __init__(self, temperature: float=1.0, lambda_d: float=0.001, lambda_q: float=0.001) -> None:
        ...
        self.document_decorrelation_loss = FrobeniusCorrelationLoss(lambda_d)
        self.query_decorrelation_loss = FrobeniusCorrelationLoss(lambda_q)
