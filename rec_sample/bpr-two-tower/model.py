...
class TwoTowerModel(nn.Module):
    ...
    def encode_user(self, x_user: torch.Tensor) -> torch.Tensor:
        return F.normalize(self.user_tower(x_user), dim=1)

    def encode_item(self, x_item: torch.Tensor) -> torch.Tensor:
        return F.normalize(self.item_tower(x_item), dim=1)

    def forward(self, x_user: torch.Tensor, x_item: torch.Tensor) -> torch.Tensor:
        u = self.encode_user(x_user)
        v = self.encode_item(x_item)
