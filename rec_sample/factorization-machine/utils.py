...
def make_movielens_user_item_features(data: RatingsData) -> tuple[np.ndarray, np.ndarray]:
    n = data.ratings.shape[0]
    d = data.num_users + data.num_items

    x = np.zeros((n, d), dtype=np.float64)
    row_idx = np.arange(n, dtype=np.int64)

    x[row_idx, data.user_indices] = 1.0
    x[row_idx, data.num_users + data.item_indices] = 1.0

    return x, data.ratings.copy()
